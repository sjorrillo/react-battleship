...
export const Battleship: React.FC<IOwnProps> = () => {
  ...
  const { setter, batteshipBoard, allowedAttemps, attemps, level, pressedCells, sinkedShips } = useBattleshipContext();
  const { positions, ships } = batteshipBoard || {};

  const handleCellClick = React.useCallback((cell: string) => {
    const shipName = batteshipBoard?.board![cell];
    const remainingSpots = shipName ? batteshipBoard?.ships![shipName].spots.filter(it => it !== cell) : null;

    const newSettings = {
      attemps: attemps! + 1,
      pressedCells: pressedCells!.concat({ name: cell, hit: !!shipName }),
      ...(remainingSpots !== null && remainingSpots.length === 0 && { sinkedShips: sinkedShips!.concat(shipName!) }),
      batteshipBoard: {
        ...batteshipBoard,
        ...(shipName && {
          ships: {
    ...
    const gameEnded = newSettings.sinkedShips?.length === Object.keys(newSettings.batteshipBoard.ships).length;
    setTimeout(() => {
  ...
  }, [batteshipBoard, attemps, allowedAttemps, pressedCells, sinkedShips, setter, openEndGameSetter, setGameResult]);

  const handleStartGame = React.useCallback((action: 'requested' | 'automatic', data?: GameStartType) => {
    if (action === 'requested') {
    ...
    setter({ batteshipBoard: newBoard, pressedCells: [], sinkedShips: [], attemps: 0, ...(data && { allowedAttemps: data.attemps } ) });
  ...
  return (
      <div className={styles.root}>
        {positions && (<Alert severity="info" classes={{ message: styles.alert }}>
          <Box sx={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}>
            <Typography>
              Level: <Typography color="primary"  component="span" sx={{ fontWeight: 'bold' }}>{level}</Typography>
            </Typography>
            <Typography>
              Remaining attemps: <Typography color="primary"  component="span" sx={{ fontWeight: 'bold' }}>{remainingAttemps}</Typography>
            </Typography>
          </Box>
        </Alert>)}
        ...
        <div className={styles.actions}>
          <Button variant="text" onClick={requestStartSetter.on}>Start New Game</Button>
          {positions && (<ToggleButtonGroup size='small' color='primary' exclusive value={showHints}  onChange={(_, value) => setShowHints(value)}>
            <ToggleButton value={true}>{showHints ? "Hide " : "Show "} Hints</ToggleButton>
          </ToggleButtonGroup>)}
        </div>

        {requestStart && (<StartNewDialog open={requestStart} gameAttemps={Number.isInteger(allowedAttemps) ? allowedAttemps as number : 0} level={level} onOk={(data) => handleStartGame('requested', data)}/>)}
        <EndGameDialog open={!!gameResult && openEndGame} type={gameResult!} attemps={attemps!} onOk={() => handleStartGame('automatic')} onClose={openEndGameSetter.off} />
      </div>
  );
};
